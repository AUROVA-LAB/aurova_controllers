...
/// input topics 
std::string objYolactTopic  =  "/yolact_ros/detections";
std::string objDepthTopic = "/depthVS_input";
std::string yolactImgTopic = "/yolact_ros/visualization";
...
void callback(const boost::shared_ptr<const yolact_ros_msgs::Detections> &yolact_data, const ImageConstPtr& depthImg_1, const ImageConstPtr& yolactImg)
...
{ 
  ...
  cv_bridge::CvImagePtr cv_ptr, cv_yolact;
  try
  { // lectura de las imagenes    
    cv_ptr = cv_bridge::toCvCopy(depthImg_1, sensor_msgs::image_encodings::MONO16);     // produndidad lidar+camara delantera
    cv_yolact = cv_bridge::toCvCopy(yolactImg, sensor_msgs::image_encodings::RGB8);  // yolo image 
  }
  ...
  yolact_ros_msgs::Detections data = *yolact_data;
  int num_obj_detect = data.detections.size();
  std::cout<<"detections: "<<num_obj_detect<<std::endl;  


  Eigen::MatrixXf xyz_object_camera(3,num_obj_detect); // lugar dodne guardo las coordenadas de los objetos 

  cv::Mat img_servo_visual  = cv_yolact->image;  // imagen servovisual que entra en el control
  ...
  for(int i=0 ;i<num_obj_detect; i++)
    ...
    std::vector<unsigned char> mask_matrix = data.detections[i].mask.mask;



    ////////////////////////////////////////////////////////////////////////////

    uint xmin = data.detections[i].box.x1;
    uint ymin = data.detections[i].box.y1;
    uint xmax = data.detections[i].box.x2;
    uint ymax = data.detections[i].box.y2;

    std::string class_name = data.detections[i].class_name;

    double score = data.detections[i].score;
    int color_r = 0;
    int color_g = 0;
    int color_b = 0;
    // color
    ...
    //std::cout<<"mask: "<<mask_matrix<<std::endl; 
    ...
    std::cout<<"Min box: "<<start_x<<", "<<end_x<<", "<<start_y<<", "<<end_y<<std::endl; 
    ...
    std::vector<int> vec_mask; // vector para mascara
    int cont_mask = -1 ;

    cv::Mat overlay;
    img_servo_visual.copyTo(overlay);
    ...
    for (uint iy = start_y;iy<end_y; iy++){
      for (uint ix = start_x;ix<end_x; ix++){
        ...
        cont_mask++;
        // Para solo usar por mascara
        if (mask_matrix[cont_mask]==0)
            continue;
        ...
        // draw mask
        cv::circle(overlay, cv::Point(ix, iy), 1, CV_RGB(color_b,color_g,color_r),cv::FILLED);
      
      }
     
    }
    // PAra evitar problemas de division entre 0
    ...
    // para poder hacer la mascara
    cv::addWeighted( img_servo_visual, 0.6,  overlay, 0.4, 0.0, img_servo_visual );
    ...
    std :: cout << "Centroide: "<<Ox<<", "<<Oy<<std::endl; 
    ...
    x_obj = (x_obj-Mcam(0,2))/Mcam(0,0) * median;
    y_obj = (y_obj-Mcam(1,2))/Mcam(1,1) * median;
    ...
    cv::rectangle(img_servo_visual, pt1, pt2,cv::Scalar(0, 255, 0),1);    // objeto detectado con profundidades
    cv::Point pt_bb1(start_x, start_y);
    cv::Point pt_bb2(end_x, end_y);
    cv::rectangle(img_servo_visual, pt_bb1, pt_bb2,cv::Scalar(0, 0, 255),1);    // bb_reducido
    //cv::putText(img_servo_visual,  cv::format("Class: %s ,D: %.2f", class_name.c_str(),depth_ave), cv::Point(xmin-20, ymin-20), cv::FONT_HERSHEY_DUPLEX, 1.0, CV_RGB(0, 0, 0), 2);  
    //cv::putText(img_servo_visual,  cv::format("Class: %.2f ,D: %.2f", score,depth_ave), cv::Point(xmin-20, ymin-20), cv::FONT_HERSHEY_DUPLEX, 1.0, CV_RGB(0, 0, 0), 2); 
  ...
  for (int j=0;j<num_obj_detect;j++)
  {
    ...
    marker.pose.position.x = xyz_object_camera(0,j)-0.15;
  ...
  image_msg->header.stamp     = yolactImg->header.stamp;
...
int main(int argc, char** argv)
{
  ...
  /// Load Parameters
  nh.getParam("/detection_mask", objYolactTopic);
  nh.getParam("/depth_lidar_image", objDepthTopic);
  nh.getParam("/yolactImg_input" , yolactImgTopic); 
  ...
  message_filters::Subscriber<yolact_ros_msgs::Detections> iaMask_sub(nh, objYolactTopic , 10);
  message_filters::Subscriber<Image>  depthImg_sub(nh, objDepthTopic, 10);
  message_filters::Subscriber<Image>  iaInimg_sub(nh, yolactImgTopic, 10);

  typedef sync_policies::ApproximateTime<yolact_ros_msgs::Detections, Image, Image> MySyncPolicy;
  Synchronizer<MySyncPolicy> sync(MySyncPolicy(100), iaMask_sub, depthImg_sub, iaInimg_sub);
  ...
  myfile.open ("/home/epvs/Escritorio/resultados_depth_estimation/resultados_segmentacion.csv");
